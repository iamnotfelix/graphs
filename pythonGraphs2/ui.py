...
from shortestWalk import shortest_walk
from dag import *
from tsp import *
...
class UI:
    ...
    def print_menu(self):
        ...
        print("18\tGet shortest walk.")
        print("19\tSolve schedule problem.")
        print("20\tSolve Travelling Salesperson Problem(TSP).")
    ...
    def get_shortest_walk(self):
        x = int(input("From: "))
        y = int(input("To: "))
        cost = shortest_walk(self.graph, x, y)
        print(cost)

    def schedule_problem(self):
        filename = input("File name: ")
        solve_schedule(filename)
    
    def print_path(self, children: list, start: int):
        if children[start] == -1:
            print(start, end=" ")
            print(0)
        else:
            print(start, end=" ")
            self.print_path(children, children[start])

    def tsp(self):
        cost, children = TSP(self.graph, 0, [0 for _ in range(0, self.graph.get_number_of_vertices())], 0,  [-1 for _ in range(0, self.graph.get_number_of_vertices())])
        print(cost)
        #print(children)
        self.print_path(children, 0)
    ...
    def command_handler(self):
        commands = {
            ...
            "18": self.get_shortest_walk,
            "19": self.schedule_problem,
            "20": self.tsp,
            "0": exit
        }
        ...
        while True:
            ...
            if command in commands:
                if (int(command) > 2 and int(command) != 19) and not self.graph:
